<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>dopda</title>
    <!-- 구글 폰트 불러오기 -->
    <!-- rotobo(400/700/900), notosanskr(400/600/900) -->
    <link
        href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&family=Roboto:wght@400;700;900&display=swap"
        rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Lato:wght@400;700&display=swap" rel="stylesheet">
    <!-- 폰트어썸 불러오기 -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.1/css/all.min.css">

    <!-- 제이쿼리 불러오기 -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
    <!-- 스와이퍼 슬라이드 불러오기 -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Swiper/5.4.5/js/swiper.min.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/Swiper/5.4.5/css/swiper.min.css">

    <script src="/PF/dopda/index.js"></script>
    <link rel="stylesheet" href="/PF/dopda/index.css">
</head>

<body>
    <section class="top-menu-box">
        <div class="flex">
            <div class="logo-box flex">
                <img src="https://kima-rin.github.io/img1/pf/dopda/logo.png" height="46" alt="">
            </div>
            <ul class="flex flex-jc-e flex-ai-c flex-grow-1">
                <li><a href="#"><span>About DOPDA</span></a></li>
                <li><a href="#"><span>SERVICES</span></a></li>
                <li><a href="#"><span>VIP MEMBERSHIP</span></a></li>
                <li><a href="#"><span>CONSULTING</span></a></li>
                <li><a href="#"><span>CONTACT</span></a></li>
            </ul>
            <div class="find-box flex flex-ai-c">
                <span><img src="https://kima-rin.github.io/img1/pf/dopda/search.png" alt=""></span>
                <span><img src="https://kima-rin.github.io/img1/pf/dopda/mobile.png" alt=""></span>
            </div>
        </div>
    </section>
    <section class="slider-box relative">
        <div class="slider-1-1">
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="slide-text">
                            <h1>For Your Business & Life</h1>
                            <p>돕다는 글로벌 컨시어지 서비스 그룹입니다. <br> 귀사와 귀하의 품격에 맞는 서비스를 제공합니다.</p>
                        </div>
                        <div class="box-1">
                            <ul class="flex">
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                            </ul>
                        </div>
                        <div class="img-box"><img src="https://kima-rin.github.io/img1/pf/dopda/main_visual01.jpg"
                                alt=""></div>
                        <div class="count-1"><img src="https://kima-rin.github.io/img1/pf/dopda/thum1.png" alt=""></div>
                    </div>
                    <div class="swiper-slide">
                        <div class="slide-text">
                            <h1>For Your Business & Life</h1>
                            <p>돕다는 글로벌 컨시어지 서비스 그룹입니다. <br> 귀사와 귀하의 품격에 맞는 서비스를 제공합니다.</p>
                        </div>
                        <div class="box-1">
                            <ul class="flex">
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                            </ul>
                        </div>
                        <div class="img-box"><img src="https://kima-rin.github.io/img1/pf/dopda/main_visual02.jpg"
                                alt=""></div>
                        <div class="count-1"><img src="https://kima-rin.github.io/img1/pf/dopda/thum2.png" alt=""></div>
                    </div>
                    <div class="swiper-slide">
                        <div class="slide-text">
                            <h1>For Your Business & Life</h1>
                            <p>돕다는 글로벌 컨시어지 서비스 그룹입니다. <br> 귀사와 귀하의 품격에 맞는 서비스를 제공합니다.</p>
                        </div>
                        <div class="box-1">
                            <ul class="flex">
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                                <li><span></span></li>
                            </ul>
                        </div>
                        <div class="img-box"><img src="https://kima-rin.github.io/img1/pf/dopda/main_visual03.jpg"
                                alt="">
                        </div>
                        <div class="count-1"><img src="https://kima-rin.github.io/img1/pf/dopda/thum3.png" alt=""></div>
                    </div>
                </div>
            </div>
            <!-- Add Pagination -->
            <div class="swiper-pagination flex flex-column-nowrap"></div>

            <ul class="bottom-text-box flex">
                <li class="flex flex-ai-c flex-1-0-0"><a href="#" class="flex flex-ai-c"><span>VIP Membership</span><img
                            src="https://kima-rin.github.io/img1/pf/dopda/arrow_btn.png" alt=""></a>
                </li>
                <li class="flex flex-ai-c flex-1-0-0"><a href="#" class="flex flex-ai-c"><span>오피스 컨시어지 서비스</span><img
                            src="https://kima-rin.github.io/img1/pf/dopda/arrow_btn.png" alt=""></a></li>
                <li class="flex flex-ai-c flex-1-0-0"><a href="#" class="flex flex-ai-c"><span>라이프 컨시어지 서비스</span><img
                            src="https://kima-rin.github.io/img1/pf/dopda/arrow_btn.png" alt=""></a></li>
                <li class="flex flex-ai-c flex-1-0-0"><a href="#" class="flex flex-ai-c"><span>정주지원 서비스</span><img
                            src="https://kima-rin.github.io/img1/pf/dopda/arrow_btn.png" alt=""></a></li>
                <li class="flex flex-ai-c flex-1-0-0"><a href="#" class="flex flex-ai-c"><span>비즈니스 투어</span><img
                            src="https://kima-rin.github.io/img1/pf/dopda/arrow_btn.png" alt=""></a></li>
            </ul>
        </div>
    </section>
    <section class="services-box">
        <div>
            <div class="img-box relative">
                <img src="https://kima-rin.github.io/img1/pf/dopda/main2_bg.jpg" alt="">
                <div class="services-text-box">
                    <section class="active-on-visible">글로벌 컨시어지 서비스, 돕다(DOPDA)와 함께 하세요.</section>
                    <img class="active-on-visible" src="https://kima-rin.github.io/img1/pf/dopda/dop_da.png" alt="">
                    <div class="line active-on-visible">
                        <span class="dot dot1"></span>
                        <span class="dot dot2"></span>
                        <span class="dot dot3"></span>
                    </div>
                    <ul class="txt-ul flex active-on-visible">
                        <li>
                            <strong>Our Solutions</strong>
                            <p>국내 최고의 비즈니스 의전 교육기관 <br>한국팔로워십센터에서 론칭한 컨시어지 서비스로<br>최상의 서비스를 제공합니다.</p>
                        </li>
                        <li>
                            <strong>Our Strengths</strong>
                            <p>국가 및 기업들의 다양한 비즈니스 행사와 <br>주요 VIP들을 대상으로 한 의전 경험으로<br>체계적인 서비스를 제공합니다.</p>
                        </li>
                        </li>
                        <li>
                            <strong>Our Visions</strong>
                            <p>글로벌 컨시어지 서비스, 돕다(DOPDA)는 <br>국내를 넘어 범아시아권에서 기준이<br>될 수 있는 컨시어지 서비스를 제공합니다.</p>
                        </li>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </section>
    <section class="premium-box con">
        <div class="pre-bg">
            <img src="https://kima-rin.github.io/img1/pf/dopda/pre_img.png" alt="">
            <h1>프리미엄 멤버십</h1>
            <p class="txt-pre">프리미엄 멤버십은 귀하만을 위한 컨시어지 서비스를 제공합니다. <br>귀하에게 최고의 삶을 선물하세요.</p>
            <p class="more">more</p>
        </div>
        <div class="img-wrap">
            <div class="pre-img1 active-on-visible"><img src="https://kima-rin.github.io/img1/pf/dopda/banner1.jpg"
                    alt=""></div>
            <div class="pre-img2 active-on-visible"><img src="https://kima-rin.github.io/img1/pf/dopda/banner2.jpg"
                    alt=""></div>
            <div class="pre-img3 active-on-visible"><img src="https://kima-rin.github.io/img1/pf/dopda/banner3.jpg"
                    alt=""></div>
            <div class="pre-img4 active-on-visible"><img src="https://kima-rin.github.io/img1/pf/dopda/banner4.jpg"
                    alt=""></div>
        </div>
    </section>
    <section class="banner-box">
        <div class="banner-text-box">
            <strong>Global Concierge Services</strong>
            <p>글로벌 컨시어지 서비스, 돕다(DOPDA)를 통해 <br> 귀사와 귀하에게 필요한 서비스를 필요한만큼만 활용하세요.</p>
            <ul class="banner-img-box flex con">
                <li class="banner-img1 active-on-visible">
                    <a href="#">
                        <div class="img">
                            <img src="https://kima-rin.github.io/img1/pf/dopda/con3_bg1.jpg" alt="">
                            <div class="hover">
                                <div class="in_ho">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100"
                                        style="enable-background:new 0 0 100 100;" xml:space="preserve">
                                        <path id="st0" class="st0"
                                            d="M28,38.9c1.8-6.1,7.4-10.5,14.1-10.5c8.1,0,14.7,6.6,14.7,14.7s-6.6,14.7-14.7,14.7c-6.5,0-12.1-4.3-14-10.2">
                                        </path>
                                        <path id="st1" class="st0" d="M51.2,61.5c-2.8,1.5-6,2.3-9.4,2.3c-11.3,0-20.4-9.1-20.4-20.4s9.1-20.4,20.4-20.4S62.3,32,62.3,43.3
										c0,5.6-2.3,10.7-6,14.4L77,78.5"></path>

                                    </svg>
                                    <span>MORE VIEW</span>
                                </div>
                            </div>
                        </div>
                        <div class="text">
                            <strong>비서 서비스</strong>
                            <span class="block">Secretarial Services</span>
                            <p>돕다는 검증과 교육을 통해 선발된 <br>
                                비서 인재풀을 보유하고 있습니다.</p>
                        </div>
                    </a>
                </li>
                <li class="banner-img2 active-on-visible">
                    <a href="#">
                        <div class="img">
                            <img src="https://kima-rin.github.io/img1/pf/dopda/con3_bg2.jpg" alt="">
                            <div class="hover">
                                <div class="in_ho">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100"
                                        style="enable-background:new 0 0 100 100;" xml:space="preserve">
                                        <path id="st0" class="st0"
                                            d="M28,38.9c1.8-6.1,7.4-10.5,14.1-10.5c8.1,0,14.7,6.6,14.7,14.7s-6.6,14.7-14.7,14.7c-6.5,0-12.1-4.3-14-10.2">
                                        </path>
                                        <path id="st1" class="st0" d="M51.2,61.5c-2.8,1.5-6,2.3-9.4,2.3c-11.3,0-20.4-9.1-20.4-20.4s9.1-20.4,20.4-20.4S62.3,32,62.3,43.3
										c0,5.6-2.3,10.7-6,14.4L77,78.5"></path>

                                    </svg>
                                    <span>MORE VIEW</span>
                                </div>
                            </div>
                        </div>
                        <div class="text">
                            <strong>수행기사 서비스</strong>
                            <span class="block">Chauffeur Services</span>
                            <p>돕다는 수준 높은 교육을 이수한 <br> 수행기사 인재풀을 보유하고 있습니다.</p>
                        </div>
                    </a>
                </li>
                <li class="banner-img3 active-on-visible">
                    <a href="#">
                        <div class="img">
                            <img src="https://kima-rin.github.io/img1/pf/dopda/con3_bg3.jpg" alt="">
                            <div class="hover">
                                <div class="in_ho">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100"
                                        style="enable-background:new 0 0 100 100;" xml:space="preserve">
                                        <path id="st0" class="st0"
                                            d="M28,38.9c1.8-6.1,7.4-10.5,14.1-10.5c8.1,0,14.7,6.6,14.7,14.7s-6.6,14.7-14.7,14.7c-6.5,0-12.1-4.3-14-10.2">
                                        </path>
                                        <path id="st1" class="st0" d="M51.2,61.5c-2.8,1.5-6,2.3-9.4,2.3c-11.3,0-20.4-9.1-20.4-20.4s9.1-20.4,20.4-20.4S62.3,32,62.3,43.3
										c0,5.6-2.3,10.7-6,14.4L77,78.5"></path>

                                    </svg>
                                    <span>MORE VIEW</span>
                                </div>
                            </div>
                        </div>
                        <div class="text">
                            <strong>의전 및 통역 서비스</strong>
                            <span class="block">Protocol Services</span>
                            <p>돕다는 귀하의 고객에게 최고의 의전과 <br> 통역 서비스를 제공할 수 있습니다..</p>
                        </div>
                    </a>
                </li>
                <li class="banner-img4 active-on-visible">
                    <a href="#">
                        <div class="img">
                            <img src="https://kima-rin.github.io/img1/pf/dopda/con3_bg4.jpg" alt="">
                            <div class="hover">
                                <div class="in_ho">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100"
                                        style="enable-background:new 0 0 100 100;" xml:space="preserve">
                                        <path id="st0" class="st0"
                                            d="M28,38.9c1.8-6.1,7.4-10.5,14.1-10.5c8.1,0,14.7,6.6,14.7,14.7s-6.6,14.7-14.7,14.7c-6.5,0-12.1-4.3-14-10.2">
                                        </path>
                                        <path id="st1" class="st0" d="M51.2,61.5c-2.8,1.5-6,2.3-9.4,2.3c-11.3,0-20.4-9.1-20.4-20.4s9.1-20.4,20.4-20.4S62.3,32,62.3,43.3
										c0,5.6-2.3,10.7-6,14.4L77,78.5"></path>

                                    </svg>
                                    <span>MORE VIEW</span>
                                </div>
                            </div>
                        </div>
                        <div class="text">
                            <strong>라이프 컨시어지</strong>
                            <span class="block">Life Concierge</span>
                            <p>돕다는 고객에게 최고의 생활 및 <br> 편의 서비스를 제공합니다.</p>
                        </div>
                    </a>
                </li>
            </ul>
        </div>
    </section>
    <section class="footer-box">
        <div class="ft-inner">
            <ul class="social flex con flex-jc-c">
                <li><a href="#"><img src="https://kima-rin.github.io/img1/pf/dopda/instagram.png" alt="" srcset=""></a>
                </li>
                <li><a href="#"><img src="https://kima-rin.github.io/img1/pf/dopda/facebook.png" alt="" srcset=""></a>
                </li>
                <li><a href="#"><img src="https://kima-rin.github.io/img1/pf/dopda/youtube.png" alt="" srcset=""></a>
                </li>
                <li><a href="#"><img src="https://kima-rin.github.io/img1/pf/dopda/blog.png" alt="" srcset=""></a></li>
            </ul>
            <div class="ft-logo flex con flex-jc-c dorder-red">
                <img src="https://kima-rin.github.io/img1/pf/dopda/ft_logo.png" alt="">
            </div>
            <div class="text">
                <p>㈜한국팔로워십센터 | 대표:이준의 | 서울시 강남구 압구정로2길 62, 4층 <br>

                    사업자등록번호: 640-87-00513 | 이메일 : dopdakorea@gmail.com <br>
                    TEL : 02-6205-8079 | FAX : 070-4324-6660 <br>
                    통신판매업번호 : 2018서울강남01915
                </p>
                <address>ⓒ 2019. ㈜한국팔로워십센터. All rights reserved.</address>
            </div>

        </div>
    </section>
    <section class="ft-banner-box">
        <div class="ft-banner">
            <strong>전세계 어디든 나만의 컨시어지가 있다. </strong>
            <span>For Your Business & Life</span>
            <a href="#"><span>Service application</span></a>
        </div>

    </section>
</body>

</html>