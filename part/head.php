<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>블로그</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.12.0-2/css/all.min.css">
    <link rel="stylesheet" href="/resource/common.css">
    <link rel="icon" href="../resource/img/icon4.png">  
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
    <script src="/resource/common.js"></script>
</head>

<body>
    
    <div class="mobile-top-bar visible-sm-down flex">
        <a href="#" class="btn-toggle-mobile-side-bar flex-as-c">
            <div></div>
            <div></div>
            <div></div>
        </a>
    </div>

    <div class="mobile-side-bar-bg visible-sm-down">    
    </div>

    <div class="mobile-side-bar visible-sm-down">
       <nav class="mobile-menu-bar">
           <ul>
              <li><a href="#" class="block">Home</a></li>
              <li><a href="#" class="block">Articles</a>
              <ul>
                  <li><a href="#">Notice</a></li>
                  <li><a href="#">Fee</a></li>
              </ul>
            </li>
              <li><a href="#" class="block">About Me</a></li>
              <li><a href="#" class="block">SNS</a>
           <ul>
              <li><a href="#" target="_block">GITHUD</a></li>
              <li><a href="#" target="_block">FACE BOOK</a></li>
              <li><a href="#">INSTAGRAM</a></li>
                </ul>
              </li>
           </ul>
       </nav>
    </div>
    
    <div class="top-bar visible-md-up">
        <div class="con height-100p flex">
        <a href="/" class="logo flex flex-ai-c"><img src="../resource/img/icon2.png" alt="icon2" width="35"></i></a>
            <nav class="menu-box-1 flex flex-1-0-0">
                <ul class="flex flex-1-0-0">
                    <li class="flex-1-0-0 flex"><a href="/" class="flex-1-0-0 flex-ai-c flex-jc-c">Home</a></li>
                    <li class="flex-1-0-0 flex"><a href="/list.php" class="flex-1-0-0 flex-ai-c flex-jc-c">Articles</a>
                    </li>
                    <li class="flex-1-0-0 flex"><a href="/aboutMe.php" class="flex-1-0-0 flex-ai-c flex-jc-c">About Me</a>
                    </li>
                    <li class="flex-1-0-0 flex"><a href="#" class="flex-1-0-0 flex-ai-c flex-jc-c">SNS</a>
                 <ul>
                     <li><a href="https://github.com/kima-rin/my_blog.git" target="_blank">GITHUD</a></li>
                     <li><a href="#" target="_blank">FACE BOOK</a></li>
                     <li><a href="#" target="_blank">INSTAGRAM</a></li>
                 </ul>
                </li>
                </ul>
            </nav>
        </div>
    </div>
    <div class="sbs-slider">
    <div class="slides">
        <div class="active" style="background-image:https://mir-s3-cdn-cf.behance.net/project_modules/max_1200/061f9e97384757.5ec3f1739958d.jpg"></div>
        <div style="background-image:"></div>
        <div style="background-color:blue;"></div>
    </div>
    
    <div class="side-bars">
        <div>
            <span>
                <i class="fa fa-angle-left" aria-hidden="true"></i>
            </span>
        </div>
        <div>
            <span><i class="fa fa-angle-right" aria-hidden="true"></i></span>
        </div>
    </div>
</div>
    



